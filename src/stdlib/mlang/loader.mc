...
lang MCoreLoader
  = Ast + DeclAst + DeclSym + TypeCheckDecl + SymGetters
...
lang MCoreLoader
  ...
  + MExprSym + MLangSym
  + MExprTypeCheck + TypeCheckMExprDecls
end
...
-- let loader = mkLoader _symEnvEmpty typcheckEnvDefault [] in
-- match includeFileExn (sysGetCwd ()) "stdlib::map.mc" loader with (symEnv, loader) in
-- utest length (getDecls loader) with 1 using lam count. lam limit. geqi count limit in
